...
TYPES = [bool, np.uint8, np.uint16, np.uint32, np.uint64, np.int8, np.int16, np.int32, np.int64, np.float32, np.float64]
    ...
    for s in SIZES:
        for t in TYPES:
            for c in COMPRESSED:
                ...
                assert np.array_equal(decode(encode(arr, c)), arr), \
